...
log = get_logger("core.file_validator")


class FileValidator:
    """Validates uploaded video and image files (extension, size, dimensions, codec)."""
    ...
    def validate_video(file_path: str | Path) -> Tuple[bool, str, Dict[str, Any]]:
        """Validate a video file. Returns (is_valid, message, metadata)."""
        log.debug("validate_video called with file_path=%s", file_path)
        path = Path(file_path)
        ...
        try:
            ...
            log.exception("validate_video failed for %s", file_path)
            return False, f"Error al procesar video: {str(e)}", {}

    @staticmethod
    def validate_image(file_path: str | Path) -> Tuple[bool, str, Dict[str, Any]]:
        """Validate an image file. Returns (is_valid, message, metadata)."""
        log.debug("validate_image called with file_path=%s", file_path)
        path = Path(file_path)
        ...
        try:
            ...
            log.exception("validate_image failed for %s", file_path)
    ...
    def calculate_hash(file_path: str | Path) -> str:
        ...
        log.debug("calculate_hash called with file_path=%s", file_path)
    ...
    def get_file_type(file_path: str | Path) -> str:
        """Return 'video', 'image', or 'unknown' based on the file extension."""
        log.debug("get_file_type called with file_path=%s", file_path)
        path = Path(file_path)
